...
describe('Survey Routes', () => {
  ...
  describe('POST', () => {
    ...
    test('Should return 204 on add survey with valid accessToken', async () => {
      await request(app)
        .post('/api/surveys')
        .set('x-access-token', await makeAccessToken(Object.assign({}, mockAccountData(), { role: 'admin' })))
        .send(makeRequestData())
        .expect(204)
    })
  })
  ...
  describe('GET', () => {
    ...
    test('Should return 200 on load survey with valid accessToken', async () => {
      ...
      await request(app)
        .get('/api/surveys')
        .set('x-access-token', await makeAccessToken(Object.assign({}, mockAccountData(), { role: undefined })))
        .expect(200)
    })
  })
